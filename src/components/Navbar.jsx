...
export const Navbar = ({ currentPage, onNavigate, isAdminAuthenticated }) => {
  ...
  return (
    <>
      <nav className="fixed top-0 left-0 right-0 z-50 bg-gradient-to-b from-[#f5ecd0]/80 via-[#ebd8a1]/35 to-transparent backdrop-blur-sm pt-3.5 pb-6 border-none shadow-none select-none text-slate-800">
    ...
    {/* Mobile Menu Drawer Overlay - Placed outside <nav> to avoid backdrop-filter containing block clipping */}
    {isOpen && (
      <div className="fixed inset-0 z-[999] lg:hidden overflow-hidden">
        ...
        <div className="fixed top-0 bottom-0 right-0 w-[300px] sm:w-[350px] max-w-[85vw] bg-[#FAF9F6] shadow-2xl p-6 flex flex-col justify-between animate-slide-in border-l border-slate-200/60 overflow-y-auto z-10">
          ...
          <div>
            <div className="flex justify-between items-center pb-4 border-b border-stone-200/60 mb-6">
              ...
              <button
                onClick={() => setIsOpen(false)}
                className="w-9 h-9 rounded-xl bg-stone-100 hover:bg-[#0F5C3B]/10 hover:text-[#0F5C3B] flex items-center justify-center cursor-pointer transition-all duration-200 border border-stone-200/40 text-slate-700"
              ...
              {[
                ...
                return (
                  <a
                    ...
                    className={`group flex items-center gap-3.5 px-4 py-3.5 rounded-2xl text-[12px] font-extrabold uppercase tracking-wider transition-all duration-200 cursor-pointer ${isActive
                      ? 'bg-[#0F5C3B] text-white shadow-md shadow-[#0F5C3B]/20 border border-[#0F5C3B]/20'
                      : 'bg-stone-50/60 text-slate-700 hover:bg-[#0F5C3B]/10 hover:text-[#0F5C3B] hover:translate-x-1.5 border border-transparent hover:border-[#0F5C3B]/20 hover:shadow-sm'
                      }`}
                  >
                    <Icon className={`w-4 h-4 transition-colors duration-200 ${isActive ? 'text-[#C9A44C]' : 'text-slate-400 group-hover:text-[#0F5C3B]'}`} />
